...
for iii=1:repeat
    ...
    for j=1:length(I)
        ...
        Variance=(states(:,1)-state)*(states(:,1)-state).'*(K0/(3+K0)+2)+processnoise;
        for i=2:7
        ...
        Py=(K0/(3+K0)+2)*(measures(1)-m_exp)*(measures(1)-m_exp).'+measurenoise;
        Pxy=(K0/(3+K0)+2)*(states(:,1)-state)*(measures(1)-m_exp).';
        for i=2:7
